...
class Transformer(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.max_case_length = dataset.max_len * self.num_event_attributes
    ...
    def call(self, x_in, training=False):
        ...
        if len(fc_in) > 0:
            x_fc = [input(x_, training=training) for x_, input in zip(fc_in, self.fc_inputs)]

            x = x_cf # tf.concat([x_cf, tf.expand_dims(x_fc, axis=1)], axis=-1)
        else:
            x = x_cf

        outputs = [o(p(x_cf))[:, (i - 1) % self.num_event_attributes::self.num_event_attributes] for i, p, o in
